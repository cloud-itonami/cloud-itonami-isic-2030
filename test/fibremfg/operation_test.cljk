(ns fibremfg.operation-test
  "Smoke tests for the compiled FibreOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `fibremfg.governor-contract-test`; the Store contract in
  `fibremfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "FibreOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "spinning-line-001" :severity :moderate :description "solvent odor near spinning line"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
