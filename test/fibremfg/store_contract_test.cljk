(ns fibremfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `fibremfg.store` ns docstring for why a second (Datomic-backed)
...
(deftest sample-data-read-basics
  (let [s (seeded)]
    ...
    (is (true? (:verified? (store/equipment-unit s "spinning-line-001"))))
    (is (true? (:registered? (store/equipment-unit s "spinning-line-001"))))
    (is (false? (:verified? (store/equipment-unit s "extrusion-line-002"))))
    (is (false? (:registered? (store/equipment-unit s "extrusion-line-002"))))
    (is (= ["extrusion-line-002" "spinning-line-001"] (mapv :id (store/all-equipment s))))
    (is (= [] (store/ledger s)))
...
(deftest fresh-store-has-no-batches-or-equipment
  (let [s (store/mem-store)]
    ...
    (is (nil? (store/equipment-unit s "spinning-line-001")))))

(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:fibre-type :nylon-6}})
    (is (= :nylon-6 (:fibre-type (store/batch s "batch-001"))))
...
(deftest maintenance-schedule-commits-and-advances-sequence
  (testing "commit-record! (like every sibling actor's own MemStore) returns the store `s`, not the domain result -- inspect the store directly, matching the discipline the actor's own :commit node relies on"
    (let [s (seeded)]
      ...
      (is (= "spinning-line-001" (:equipment-id (store/maintenance s "mnt-1"))))
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-weight
  (let [s (seeded)]
    ...
    (is (= 13000.0 (:shipped-weight-kg (store/batch s "batch-001")))
        "8000.0 seeded + 5000.0 committed")))
