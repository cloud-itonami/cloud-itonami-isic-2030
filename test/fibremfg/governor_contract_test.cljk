(ns fibremfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the spinning/extrusion line
  directly... does NOT authorize or execute maintenance/line
  operations') implemented faithfully. The single invariant under
  test:

    FibreAdvisor never schedules maintenance, flags a safety concern,
    or coordinates a shipment the Man-Made Fibre Plant Operations
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:fibre-type :polyester}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :polyester (:fibre-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-spinning-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest invalid-fibre-type-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:fibre-type :unobtainium}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-fibre-type} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium (:fibre-type (store/batch db "batch-001"))) "fabricated fibre-type never lands in the SSoT")))

(deftest invalid-denier-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:denier -5.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-denier} (-> (store/ledger db) last :basis)))
    (is (not= -5.0 (:denier (store/batch db "batch-001"))) "fabricated denier never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "spinning-line-001" :severity :moderate
                                            :description "solvent odor near spinning line"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "spinning-line-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
  ...
  (testing "write-only-through-ledger: N settled operations -> N ledger facts"
    (let [[db actor] (fresh)]
      ...
      (exec-op actor "b" {:op :log-production-batch :effect :propose :subject "batch-001"
                          :patch {:fibre-type :fabricated-type}} coordinator)
