(ns fibremfg.governor
  "Man-Made Fibre Plant Operations Governor -- the independent
  compliance layer that earns the FibreAdvisor the right to commit.
  ...
  draft-schedule) the spinning/extrusion line, whether a shipment
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:line/actuate` or
                                       `:spindle/run`) is the 'direct
                                       spinning/extrusion-line-
    ...
    4. Line-actuate blocked        -- for `:schedule-maintenance`, does
                                       ...
                                       spinning/extrusion line is this
    ...
    8. Shipment weight exceeded    -- for `:coordinate-shipment`,
                                       ...
                                       the batch's own recorded
                                       `:weight-kg`
                                       (`fibremfg.registry/shipment-
                                       weight-exceeded?`) -- ground
                                       truth from the batch's own
                                       permanent fields, never a
                                       self-reported weight claim.
    9. Invalid fibre-type          -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:fibre-type` outside the closed
                                       known set (`fibremfg.registry/
                                       fibre-type-valid?`), the batch
                                       record is rejected rather than
                                       let a fabricated fibre type
                                       through.
   10. Invalid denier              -- for `:log-production-batch`, if
                                       the patch declares a `:denier`
                                       that is not a physically
                                       plausible fineness reading
                                       (`fibremfg.registry/denier-
   ...
   11. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [fibremfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  spinning/extrusion-line-equipment-control effect."
...
(defn- line-control-blocked-violations
  ...
  Anything else (direct spinning/extrusion-line control, a fabricated
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :line-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は紡糸・押出設備等の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- line-actuate-blocked-violations
  ...
  directly actuate the spinning/extrusion line -- this actor may only
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :line-actuate-blocked
      :detail "紡糸・押出ラインの直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- invalid-fibre-type-violations
  "For `:log-production-batch`, if the patch declares a `:fibre-type`
  outside the closed known set, reject rather than let a fabricated
  fibre type through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [fibre-type (:fibre-type (:value proposal))]
      (when (and (some? fibre-type) (not (registry/fibre-type-valid? fibre-type)))
        [{:rule :invalid-fibre-type
          :detail (str fibre-type " は既知の fibre-type 値ではない")}]))))

(defn- invalid-denier-violations
  "For `:log-production-batch`, if the patch declares a `:denier` that
  is not a physically plausible fineness reading, reject rather than
  let fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [d (:denier (:value proposal))]
      (when (and (some? d) (not (registry/denier-valid? d)))
        [{:rule :invalid-denier
          :detail (str d " デニールは物理的に妥当な範囲外")}]))))

(defn check
  "Censors a FibreAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-fibre-type-violations request proposal)
                           (invalid-denier-violations request proposal)))
