(ns fibremfg.operation
  "FibreOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (FibreAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Man-Made Fibre Plant
...
(defn build
  "Compiles a FibreOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `fibremfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FibreAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Man-Made Fibre Plant Operations Governor -- independent censor.
      (g/add-node :govern
