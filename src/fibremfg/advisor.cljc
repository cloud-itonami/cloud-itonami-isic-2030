(ns fibremfg.advisor
  "FibreAdvisor -- the *contained intelligence node* for the
  man-made-fibre plant-operations coordination actor.

  It normalizes production-batch patches (fibre-type/weight/denier/
  tenacity), drafts a spinning/extrusion-line maintenance scheduling
  proposal against a piece of equipment, drafts a safety-concern flag,
  and drafts an outbound man-made-fibre shipment coordination proposal
  ...
  cited), never a committed record and NEVER a real spinning/
  extrusion-line actuation or freight dispatch. Every output is
  censored downstream by `fibremfg.governor` before anything touches
  the SSoT -- see README `What this actor does NOT do`.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; spinning/extrusion-line-control
                                 ; effect
  ...
  contract every caller of this actor agrees to) -- `fibremfg.governor`
...
(defn- log-production-batch
  ...
  validates the patch; it does not invent the batch's fibre-type,
  weight, denier or verification status. High confidence, low stakes
...
(defn- schedule-maintenance
  "Draft a spinning/extrusion-line maintenance-window scheduling
  ...
  `fibremfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft a chemical-hazard (solvent-exposure)/equipment-safety
  ...
  technicality). See `fibremfg.phase`: no phase ever adds this op to a
  phase's `:auto` set; `fibremfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound man-made-fibre shipment coordination proposal
  against a production batch. The advisor passes through the
  caller's own claimed weight -- it does NOT invent one, and
  `fibremfg.governor` NEVER trusts it: it independently recomputes
  whether the batch's own cumulative-shipped weight plus this claim
  would exceed the batch's own recorded weight before any commit is
  possible."
  [db {:keys [subject value]}]
...
(def ^:private system-prompt
  (str "あなたは化学繊維(man-made fibre)工場プラント運用コーディネーターの"
       "助言者です。"
       ...
       "紡糸・押出ライン等の設備の直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `fibremfg.governor`
