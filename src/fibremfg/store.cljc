(ns fibremfg.store
  "SSoT for the man-made-fibre plant-operations coordination actor,
  ...
  Scope note: like its closest domain sibling
  (`cloud-itonami-isic-2013`'s own `resinmfg.store`), this build ships
  ...
  man-made-fibre-manufacturing capability library, no
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             fibre-type/weight/denier record.
    ...
    - `equipment`         -- a spinning-line/extrusion-line unit's own
                             record. `:verified?`/`:registered?` track
                             whether it has actually been
                             inspected/commissioned and is on file --
                             the same ground-truth discipline as
                             `batches`.
    - `maintenance`       -- a scheduled spinning/extrusion-line
                             maintenance-window DRAFT against a piece
                             of equipment (`fibremfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound man-made-fibre
                             shipment DRAFT (`fibremfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (fibremfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (fibremfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :fibre-type :polyester :output-form :filament
                :material "Polyester Continuous Filament Yarn"
                :weight-kg 40000.0 :denier 150.0 :tenacity-cn-per-tex 45.0
                :verified? true :registered? true
                :shipped-weight-kg 8000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :fibre-type :viscose-rayon :output-form :staple
                :material "Viscose Rayon Staple Fibre"
                :weight-kg 8000.0 :denier 1.5 :tenacity-cn-per-tex 22.0
                :verified? true :registered? true
                :shipped-weight-kg 7500.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :fibre-type :nylon-66 :output-form :tow
                :material "Nylon 66 Industrial Tow"
                :weight-kg 60000.0 :denier 300000.0 :tenacity-cn-per-tex 60.0
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"spinning-line-001" {:id "spinning-line-001" :kind :spinning-line
                         :verified? true :registered? true
                         :last-maintenance-date "2026-05-01"}
   "extrusion-line-002" {:id "extrusion-line-002" :kind :extrusion-line
                          :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `fibremfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `fibremfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered spinning-line unit
  (schedulable for maintenance), one UNVERIFIED/unregistered
  extrusion-line unit (blocks any maintenance scheduling against it)
  -- so the actor + demo + tests run offline. Returns `s`
