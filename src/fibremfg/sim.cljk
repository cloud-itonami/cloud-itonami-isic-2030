(ns fibremfg.sim
  ...
  production weight, a proposal that tries to ACTUATE the
  spinning/extrusion line directly (permanently blocked, no override),
  a double-schedule of the same maintenance window, a production-batch
  patch with a fabricated fibre type, and a production-batch patch
  with an implausible denier reading.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on spinning-line-001 (verified, registered, spinning line -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on spinning-line-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on extrusion-line-002 (UNVERIFIED/unregistered extrusion line -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on spinning-line-001 with :actuate-line? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with a fabricated fibre-type -> HARD hold ==")
    (println (exec-op actor "t12"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:fibre-type :unobtainium}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible denier reading -> HARD hold ==")
